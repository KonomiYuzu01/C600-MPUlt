...
C600 Studio adds state management and solving tools around Andrey Astrelin's Magic Puzzle Ultimate. Its full-model journal, certified operations, inspection, and structure navigation provide more than additional keybindings. These are descriptions of project capabilities and proposed work, not claims of historical priority over other puzzle software. Upstream credit and runtime boundaries are documented in runtime provenance (`RUNTIME_PROVENANCE.md`; local-only reference).
...
Inspect current implementation and tests before extending a feature. The structure guide (`STRUCTURE_EXPLORER.md`; local-only reference) is the authoritative user-facing contract for color and layer notation. Older zero-based filter notation must remain compatible.
...
## Planned versions 0.4 and 1.0

Complete buffer preparation and completed-orbit protection integration in **0.4** first. The subsequent **1.0** objective is a validated modern frontend migration: retain the functioning legacy shell for continuity and fallback, evaluate HSC/HSC2-informed interaction and suitable reusable components, and select a frontend only after full-model Windows validation. The old shell is not a permanent requirement. See the [migration conditions](NEXT_UPDATE.md#version-10-frontend-migration-readiness). This is planned work, not a completed frontend replacement or a new performance claim.
