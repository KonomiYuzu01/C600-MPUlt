...
# Version 0.4: human-directed orbit-first preparation

This is the implementation and handoff contract for the next update. It is an **unreleased, isolated foundation** based on C600 Studio 0.3. The next planned release is **0.4**; the subsequent modern-frontend milestone is **1.0**. It does not add a preparation UI, HTTP endpoint, or executable feature to the published 0.3 package.
...
## Version 1.0: frontend migration readiness

Version **0.4** remains focused on the human-directed buffer preparation and completed-orbit protection workflow. For version **1.0**, the main development objective is to retain the working legacy shell while finding and validating a suitable modern frontend for migration. Retention provides continuity, a comparison reference and a fallback; it does not make WinForms or the MPUlt viewport a permanent architectural requirement.

Use HSC and HSC2 as references for an integrated hypercubing interaction experience, not merely a visual theme. Compare actual target lookup, grip/keybind feedback, filtering, buffer/frame preparation, protection review and explicit execution workflows. A new frontend must preserve expert capability and reduce preparation and verification effort; ease of learning remains an important supporting goal.

Evaluate reuse/adaptation of suitable HSC components and an independently implemented frontend against the same requirements. Do not preselect D3D11, wgpu, egui or a complete engine rewrite. Check current upstream implementation, supported hardware, licensing, attribution and maintenance boundaries before choosing. Relevant starting points are the [HSC repository](https://github.com/HactarCE/Hyperspeedcube) and its [architecture notes](https://github.com/HactarCE/Hyperspeedcube/blob/main/crates/README.md); the latter warns that parts are outdated. No HSC/HSC2 hands-on comparison or full-model compatibility test is claimed here.

Migration gates:

- Use the complete immutable model, all 259,800 sticker slots and 1,200 legal generators, with verified stable IDs, projection/clipping and exact picking. Preserve original MPUlt binaries and attribution.
- Keep the existing authoritative state, preparation analysis, preview/commit checks, journal and recovery. Rendering and animation never authorize a move or create a second mutable puzzle authority.
- Demonstrate a usable human-directed preparation cycle with spatial grip/keybind feedback, composable filters, linked views, visible protection conflicts and correct input focus. Preserve existing effective features and advanced controls.
- Compare real Windows rendering and interactions at matching view, size and detail, including Intel HD 620. Measure CPU/GPU stages, full-detail restoration, committed-turn latency and resource use. Full-detail 30 FPS at 1920 x 1080 and 100 ms p95 committed-turn latency remain targets, not promises or established hardware capability.
- Verify focused full-state agreement, undo/checkpoint/import/reopen, stale-request rejection, hidden-piece noninteraction and rendering failure recovery. A renderer change cannot be assumed to fix backend transaction latency.
- Keep alternative architectures and benchmarks isolated. Promote only changes with measured benefit, preserved correctness/recovery and a smaller, maintainable production implementation. Discard failed experiments; retain only concise measurements and limitations in documentation.
- Retain a working legacy launch path during migration. Switch frontends only at a coherent committed-state boundary, with no concurrent writers; never silently discard a pending operation or change preferences. Release promotion requires matching validation and an explicit release decision.

The 1.0 work begins with a tested frontend candidate, an evidence-based migration decision and a staged compatibility/fallback plan. Shipping 1.0 requires the selected frontend to be integrated and validated against these gates; research or a static prototype alone does not complete the release. It is not another round of cosmetic changes to the old shell. No replacement code or release is introduced by this roadmap update.

## Reuse the existing architecture

For the immediate preparation update, keep one authoritative Model, PuzzleState, Session, session lock, owned EngineProcess, and journal. Keep the original MPUlt viewport and current revisioned rendering protocol. No additional solver process, database, event bus, rendering engine, generic plugin framework, or duplicate permutation implementation is needed.
