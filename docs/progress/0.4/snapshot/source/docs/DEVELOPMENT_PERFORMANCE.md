...
The current release target has not been fully met: 100 ms instant turns and continuous full-detail 30 FPS remain unresolved, and one short explorer activation sample narrowly missed 50 ms. The measurements establish a more responsive working interface, not a claim that all performance requirements passed. See the [development log](DEVELOPMENT_LOG.md), structure guide (`STRUCTURE_EXPLORER.md`; local-only reference), and [limitations](LIMITATIONS_AND_ROADMAP.md).
