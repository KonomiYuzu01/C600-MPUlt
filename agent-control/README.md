...
Create an application API key in the project's [OpenAI Platform](https://platform.openai.com/) account with `api.agents.read`, `api.agents.write` and `api.responses.write`, as specified in the [official quickstart](https://developers.openai.com/api/docs/guides/agents-api/quickstart). If these capabilities are unavailable, ask the project administrator to enable access. Local configuration cannot grant account permissions.
...
```powershell
powershell -NoProfile -File .\configure-key.ps1
```

Paste the issued key at the hidden prompt. The script saves it to the local Git-ignored `.env`, preserves other template settings, and refuses a OneDrive directory. It does not verify account access. Git ignore alone does not prevent cloud-folder synchronization.

Alternatively, supply the key only to the current PowerShell process:

```powershell
$magic600Secret = Read-Host 'Paste your OpenAI Platform API key' -AsSecureString
$magic600Credential = [System.Net.NetworkCredential]::new('', $magic600Secret)
$env:OPENAI_API_KEY = $magic600Credential.Password
Remove-Variable magic600Secret, magic600Credential
$env:OPENAI_MODEL = 'gpt-6-astra'
```

Run subsequent commands in that terminal; this does not update another already-running Codex process. For an external environment file, copy `.env.example` to a local location outside cloud synchronization, such as `$env:LOCALAPPDATA\Magic600\agent-control.env`. Replace its empty key locally with the real value:
